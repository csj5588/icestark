...
import { Form, Input, Select, Menu, Dropdown, Icon, Modal } from 'antd';
...
class Content extends React.Component {
  ...
  handleDeleteEnv = index => {
    confirm({
      ...
      onOk: () => {
        ...
        const { domain_uris: domainUri } = createParams || {}
        const newDomainUrl = _cloneDeep(domainUri)
        newDomainUrl.splice(index, 1)
        dispatch(saveCreateParams({ domain_uris: newDomainUrl }));
      },
      onCancel: () => {
        console.log('取消');
      },
  ...
  renderAdd = () => {
    ...
    const showAdd = isEdit && addEnvList && addEnvList.length > 0;
    const menu = (
          ...
          <div>
            ...
            </Form.Item>
            {
              domainUri && domainUri.map((item, index) => {
                const { env, domain_id: domainId, is_add: isAdd, uri } = item
                ...
                return [
                  <Form.Item key={index + 'uri'} label={`${env}环境uri`} {...formItemLayout}>
                    ...
                    {isAdd ? (<Icon className="icon-del" type="minus-circle" onClick={() => this.handleDeleteEnv(index)} />) : null}
                  </Form.Item>,
