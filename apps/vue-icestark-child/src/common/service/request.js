...
class Request {
  ...
  getServiceParams (serviceParams) {
    ...
    if (options.headers && (
      options.headers['Content-Type'] === serviceConfig.xFormOptions.headers['Content-Type']
    )) {
    ...
    if (isShort) {
      ...
      */
      if (serviceParams.url && serviceParams.url.substring(serviceParams.url.length - 1) === '/') {
  ...
  getRequestFunc (key, apiConfig) {
    return (data, options, settings) => {
      ...
      const serviceParams = this.getServiceParams(commonUtil.copy(true, { data, options, settings }, apiConfig))
      let { url, type, data: _data, options: _options, settings: _settings } = serviceParams        
      let isShort = this.shortParamsList.indexOf(type) > -1
      ...
      return new Promise((resolve, reject) => {
        ...
        if (typeof $httpFunc === 'function') {
          ...
          return $httpFunc.call($http, url, !isShort ? _data : _options, _options)
          .then((res) => {
  ...
  getApiCollection (apis) {
    ...
    if (!this.$http) throw new Error('Service hasn\'t setReqInstance, Pelease var service = new Service() and service.setReqInstance(http)')
  ...
  _beforeSendSub () {
    observer.subscribe(events.beforeSend, ({ serviceParams, key, apiConfig }) => {
      let { autoLoading } = serviceParams.settings
      
      autoLoading && loading.show('service')
  ...
  _afterSendSub () {
    observer.subscribe(events.afterSend, ({serviceParams, res}) => {
      let { autoLoading } = serviceParams.settings
      autoLoading && loading.hide('service')
    })
  }
